...
exports.Pool = function (factory) {
  var me = {},
      ...
      connectionTimeOut = factory.connectionTimeOut || 30000,
  ...
  ///////////////
     ...
     * @param obj
     */
    me.pushUsingObjects = function(obj) {
        obj.last_use_time=new Date().getTime();
        usingObjects.push(obj);
    };
     ...
     * @param obj
     */
    function clearTimeOutUsing() {
        //console.log("---------------------start to clearTimeOutUsing----usingObjects.length:"+usingObjects.length);
        var now = new Date().getTime();
        var toRemove=[];
        for(i = 0; i < usingObjects.length; i += 1){
            if(now >= (usingObjects[i].last_use_time + connectionTimeOut)){
                toRemove.push(usingObjects[i]);
            }
        }
        //console.log("---------------------dddd to clearTimeOutUsing----toRemove.length:"+toRemove.length);
        for (i = 0; i < toRemove.length;  i += 1) {
            me.destroy(toRemove[i]);
        }

        for (i = 0; i < toRemove.length;  i += 1) {
            usingObjects = usingObjects.filter(function(usingObject) {
                return (usingObject.t != toRemove[i].t);
            });
        }
        //console.log("---------------------end to clearTimeOutUsing----usingObjects.length:"+usingObjects.length);
    };

    //定时清理
    setInterval(clearTimeOutUsing,60000);
  ...
  me.destroy = function(obj) {
    ...
    availableObjects = availableObjects.filter(function(objWithTimeout) {
              return (objWithTimeout.obj.t != obj.t);
    });
  ...
  function adjustCallback(cb, err, obj) {
    ...
    function validateConnection(connection) {
        try{
            if(connection.connection.writable==false){
                me.destroy(connection);
                return false;
            }
        }catch(err){
            return false;
    ...
    try{
        if (waitingCount > 0) {
            while (availableObjects.length > 0) {
                ...
                if(!validateConnection(objWithTimeout.obj)){
                    continue;
                }
  ...
  me.acquire = function (callback, priority) {
    if (draining) {
      ...
      //console.log("--count:"+count+",max"+factory.max+",availableObjects="+availableObjects.length+",usingObjects="+usingObjects.length);
      if(count>=factory.max){
          return false;
      }
  ...
  me.release = function (obj) {
    ...
    if (availableObjects.some(function(objWithTimeout) { return (objWithTimeout.obj.t === obj.t); })) {
      log("release called twice for the same resource: " + (new Error().stack), 'error');
      return;
    }
    //log("return to pool");
      //console.log("----------------------11--realse,usingObjects.lenght:"+usingObjects.length);
      usingObjects = usingObjects.filter(function(usingObject) {
          return (usingObject.t != obj.t);
      });
    //console.log("----------------------22--realse,usingObjects.lenght:"+usingObjects.length+",obj.t="+obj.t);

    var objWithTimeout = { obj: obj, timeout: (new Date().getTime() + idleTimeoutMillis) };
    //console.log("obj.is_end="+obj.is_end);
      if(obj.is_end === false){
          if(returnToHead){
